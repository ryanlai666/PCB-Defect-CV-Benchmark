...
models/deimv2_l.py
==================
Model wrapper for DEIMv2-L (DINOv3-S backbone) on the DeepPCB dataset.

This module follows the project's model-registry pattern (build() function
returning an object whose .train() method kicks off training). Because DEIMv2
has its own training engine (engine/ + train.py launched via torchrun), this
wrapper delegates to a subprocess rather than the standard training.py loop.

The wrapper is classified as a DEIMV2_MODEL in train_model.py, which handles
the subprocess call.
"""
...
CONFIG_PATH = DEIMV2_DIR / "configs" / "deimv2" / "deimv2_dinov3_l_deeppcb.yml"
...
BACKBONE_DST = CKPTS_DIR / "dinov3_vits16_pretrain_lvd1689m-08c60483.pth"

MODEL_NAME   = "deimv2_l"
VARIANT_LABEL = "DEIMv2-L (DINOv3-S backbone)"
...
class DEIMv2Wrapper:
    """
    Thin wrapper that exposes the same interface as other project models.

    train_model.py detects this class and calls subprocess-based training
    instead of the standard PyTorch training loop.
    """
...
def build(num_classes: int = 6, pretrained: bool = True, **kwargs):
    ...
    if not CONFIG_PATH.exists():
        raise FileNotFoundError(
            f"DEIMv2-L config not found: {CONFIG_PATH}\n"
